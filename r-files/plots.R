...
alberta_map(data = data, filename = "plots/Fig_1_map", not_surveyed=not_surveyed)

#----------------------
#Visualizing Paloheimo's effects
sub_dat <- data %>% filter(name=="buck lake")
yr_dat <- sub_dat$year
sub_dat <- sub_dat[, c( which(colnames(sub_dat) %in% 2:25))]
sub_dat$year = yr_dat
sub_dat <- sub_dat %>% pivot_longer(-year, names_to="age") %>%
  mutate(year = year + 1999)

sub_dat$age <- as.integer(sub_dat$age)
sub_dat$value[sub_dat$value == 0] <- NA
bp <- ggplot(sub_dat, aes(x=year, y=age, size=value)) + geom_blank() 
...
bp <- bp + annotate(geom="text", x=2004, y=19, label="Age \n effects", col="black")
bp <- bp + annotate(geom="text", x=2015, y=17, label="Recruitment \n effects", col="black")
bp <- bp + annotate(geom="text", x=2015, y=2.3, label="Year \n effects", col="black")
