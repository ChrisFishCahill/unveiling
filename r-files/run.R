...
get_fits <- function(rec_model = c("bev-holt", "ricker"), 
  ...
  for (i in 1:((length(names) / 2)+1)) {
    ...
    # create a file name and find the correct directory
    # note: files were big, so saved on an external hard drive
    #Beverton-holt or ricker:
    if (rec_model == "ricker") {
      stan_file <- paste0("D:/unveiling_fits/ricker//fit_", which_fit, ".rds")
    }
    if (rec_model == "bev-holt") {
      stan_file <- paste0("D:/unveiling_fits/bev_holt//fit_", which_fit, ".rds")
    }

    # # Add in compensation ratio identifier for fit
    if (stan_data$cr_prior == 12) {
      ...
      stringi::stri_sub(stan_file, from = which_idx[1]+1,
                        to = which_idx[2]-1) <- "cr12"
    }
    if (stan_data$cr_prior == 6) {
      which_idx <- stringr::str_locate_all(pattern = "//", stan_file)[[1]]
      stringi::stri_sub(stan_file, from = which_idx[1]+1,
                        to = which_idx[2]-1) <- "cr6"
    }
    ...
    #easier to get the tableaus now rather than later
    tryCatch(suppressMessages(get_tableau(
      fit = fit, data = run_data,
      stan_data = stan_data,
      stocking = run_stocking
    )),
    error = function(e) {
      #if buggered, write a value to global env to debugger
      which_fail <- c()
      which_fail <<- c(which_fail, which_lakes)
    }
    )
    counter <- counter + 2
