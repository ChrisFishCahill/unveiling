...
data {
  ...
  real<lower=0> v_f_ctl;               // Swimming speed contact vul(0) or knife edge(1)
  real<lower=0> a_inflect;             // 50% vul when v_f_ctl==1
  real<lower=0> delta;                 // dif between 50% & 95% vul when v_f_ctl==1
  real<lower=0> m_prior;               // prior for depensation 
}
transformed data {
  ...
  for(k in 1:n_lakes){
    ...
    for(a in 1:n_ages){
      ...
      if(v_f_ctl==0){ //swimming speed
        v_f_a[k, a] = ((linf[k]/lbar)*(1 - exp(-vbk[k] * ages[a])))^psi;  
      }
      if(v_f_ctl==1){ //logistic
        v_f_a[k, a] = 1 / (1 + exp(-log(19)*(ages[a]-a_inflect)/delta));  
      }
...
parameters {
  ...
  vector<lower=0>[n_lakes] m;                            // Lake specific depensation parameters
...
transformed parameters {
  ...
  for(k in 1:n_lakes){
    ...
    for(t in 3:n_years){
      if(rec_model==0){//depensatory ricker
        Nat_array[1, t, k] = (SSB[k, t-2]^m[k])*exp(ar[k] - br[k]*SSB[k, t-2] + w[k,t-2]);
      }
      if(rec_model==1){//depensatory beverton-holt
        Nat_array[1, t, k] = (SSB[k, t-2]^m[k])*exp(ar[k] + w[k,t-2]) / (1 + br[k]*SSB[k, t-2]);
      }
      for(a in 2:n_ages){
